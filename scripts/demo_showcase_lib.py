...
Shared by every showcase scenario under scripts/scenarios/.
"""
...
# wombatkv-daemon binary path. The wombatkv repo lives separately; set
# WOMBATKV_DAEMON_BIN=/abs/path/to/wombatkv-daemon, or set WOMBATKV_DIR=
# to the wombatkv repo checkout root and we'll look for
# `$WOMBATKV_DIR/target/release/wombatkv-daemon`.
def _resolve_wombatkv_daemon():
    explicit = os.environ.get("WOMBATKV_DAEMON_BIN")
    if explicit:
        return pathlib.Path(explicit)
    repo = os.environ.get("WOMBATKV_DIR")
    if repo:
        return pathlib.Path(repo) / "target" / "release" / "wombatkv-daemon"
    # Fall back to a sibling `wombatkv/` checkout (common dev layout).
    return DS4_DIR.parent / "wombatkv" / "target" / "release" / "wombatkv-daemon"

WOMBATKV_DAEMON_BIN = _resolve_wombatkv_daemon()
