...
DS4_DIR = Path(os.environ.get("DS4_DIR", Path(__file__).resolve().parent.parent))
DS4_BIN = DS4_DIR / "ds4-server"
MODEL = "gguf/DeepSeek-V4-Flash-IQ2XXS-w2Q2K-AProjQ8-SExpQ8-OutQ8-chat-v2-imatrix.gguf"

# wombatkv-daemon lives in the wombatkv repo checkout. Set
# WOMBATKV_DAEMON_BIN explicitly, or WOMBATKV_DIR to the repo root.
WOMBATKV_DIR = Path(os.environ.get("WOMBATKV_DIR", DS4_DIR.parent / "wombatkv"))
DAEMON_BIN = Path(
    os.environ.get("WOMBATKV_DAEMON_BIN", WOMBATKV_DIR / "target" / "release" / "wombatkv-daemon")
)
