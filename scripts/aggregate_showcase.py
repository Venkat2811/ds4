...
def summarize(rows, trials):
    ...
    def stats(pop, field):
        xs = [
            r[field]
            for r in pop
            if isinstance(r[field], (int, float)) and r[field] == r[field]
        ]
        if not xs:
    ...
    cold_walls = sorted(
        {r["wall_ms"] for r in cold if isinstance(r["wall_ms"], (int, float))}
    )
    warm_walls = sorted(
        {r["wall_ms"] for r in warm if isinstance(r["wall_ms"], (int, float))}
    )
    cold_wall = cold_walls[0] if cold_walls else float("nan")
    warm_wall = median(warm_walls) if warm_walls else float("nan")

    return {
        "cold_ttft_med": cold_ttft_med,
        "cold_ttft_p95": cold_ttft_p95,
        "warm_ttft_med": warm_ttft_med,
        "warm_ttft_p95": warm_ttft_p95,
        "cold_total_med": cold_total_med,
        "warm_total_med": warm_total_med,
...
def main():
    ...
    mode_order = [
        m for m in ("c1_native", "c2_embedded", "c3_daemon") if m in modes_in_results
    ]
        ...
        for metric in (
            ...
            if "c1_native" in mode_order and len(mode_order) >= 2:
                ...
                for mode in mode_order:
                    if mode == "c1_native":
                        continue
                    cand = summaries.get((mode, scen), {}).get(metric, float("nan"))
                    if metric == "warm_cached_tokens_med":
                        ...
                        speedups.append(
                            f"{mode}={_fmt(sp)}x" if sp == sp else f"{mode}=-"
                        )
    ...
    md.append(
        "  - `cold_*` = trial 1 (first request after a fresh bucket "
        "and fresh ds4-server)."
    )
    md.append(
        "  - `warm_cached_tokens_med` = median cached_tokens reported "
        "by ds4 in the SSE usage object; non-zero confirms a "
        "Tier A / Tier B cache hit landed on the server side."
    )
    md.append(
        "  - `*_wall_ms` is the full-trial wall clock "
