...
def run_mode(mode, outdir, trials):
    ...
    env = lib.env_for_mode(mode, puffer_dir=puffer, bucket=bucket,
                           daemon_prefix=daemon_prefix)
    log_path = logs_dir / f'ds4-server-port{port}.log'
    lib.log(f'  starting ds4-server :{port}  ({mode})')
    server_proc = lib.start_server(env, port=port,
                                   kvdisk=kvdisk, log_path=log_path)

    try:
        all_trial_results = []
        for trial in range(1, trials + 1):
            lib.log(f'  trial {trial}/{trials}: running {len(CONVERSATIONS)} '
                    'conversations sequentially')
            t_trial_start = time.perf_counter()
            conv_results = []
            for conv in CONVERSATIONS:
                lib.log(f'    conversation: {conv["name"]} '
